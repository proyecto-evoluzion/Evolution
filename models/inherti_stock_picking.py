...
class InheritStockPicking(models.Model):
    ...
    def quant_qty_compare(self, actual_qty, to_proccess):
        if actual_qty > to_proccess:
            return True
        else:
            return False

    # def stock_lot_quant(self):
    #     for moves in self.move_lines:
    #         count = 0
    #         difference = 0
    #         lot_obj = self.env['stock.production.lot'].search([('product_id','=',moves.product_id.id)])
    #         quant_obj = self.env['stock.quant'].search(['&',('product_id','=',moves.product_id.id),('location_id.usage','=', 'internal')])
    #         print('Lenght Lote')
    #         print(len(quant_obj))
    #         for quant in quant_obj:
    #             print('LOTE LOTE LOTE LOTE')
    #             print(quant.lot_id)
    #             count += 1
    ...
    #                 if difference == 0:
    #                     print('difference == 0')
    #                     compare = self.quant_qty_compare(available, moves.quantity_done)
    #                     if compare:
    #                         # quant.update({'reserved_quantity': quant.reserved_quantity + moves.quantity_done})
    #                         for lines in moves.move_line_ids:
    #                             lines.update({'lot_id': quant.lot_id.id, 'qty_done': moves.quantity_done, 'product_uom_qty': moves.quantity_done})
    #                         break
    #                     difference = moves.quantity_done - available
    #                     # quant.update({'reserved_quantity': quant.reserved_quantity + available})
    #                     for lines in moves.move_line_ids:
    #                             lines.update({'lot_id': quant.lot_id.id, 'qty_done': available, 'product_uom_qty': quant.reserved_quantity + available})
    #                 else:
    #                     lot_vals = {}
    #                     if count <= len(quant_obj):
    #                         compare = self.quant_qty_compare(available, difference)
    #                         if compare:
    #                             # quant.update({'reserved_quantity': quant.reserved_quantity + difference})
    #                             print(moves.product_id.uom_id)
    #                             print(moves.product_id.uom_id.id)
    #                             lot_vals = {
    ...
    #                                 'qty_done': difference,
    #                                 'reference': moves.picking_id.id,
    #                                 'state': 'done',
    #                                 'product_uom_id': 1,
    #                                 'move_id': moves.id,
    #                             }
    #                             self.env['stock.move.line'].create(lot_vals)
    #                             break
    ...
    #                                 'location_dest_id': moves.location_dest_id.id,
    #                                 'lot_id': quant.lot_id.id,
    #                                 'product_uom_qty': moves.quantity_done,
    #                                 'qty_done': difference,
    #                                 'reference': moves.picking_id.id,
    #                                 'state': 'done',
    #                                 'product_uom_id': moves.product_id.uom_id.id,
    #                             }
    #                             line_id = lines.create(lot_vals)
    #                     if count == len(quant_obj):
    #                         for lines in moves.move_line_ids:
    #                             if lines.id == line_id:
    #                                 lines.update({'qty_done': available + difference, 'state': 'assigned'})
    #                                 moves.update({'no_available_lot': 1})
    #             if count == len(quant_obj):
    #                 difference = 0
    #                 for lines in moves.move_line_ids:
    #                     lines.update({'qty_done': 0, 'state': 'assigned'})
    #                     copy_move_obj = self.env['copy.stock.move'].search([('product_id','=',moves.product_id.id),('picking_id','=',moves.picking_id.id)], limit=1)
    #                     copy_move_obj.update({'no_available_lot': 1})
...
class InheritStockMove(models.Model):
    ...
    def create(self, vals):
        ...
        if res.product_id.name != 'MATERIALES DE CIRUGÍA':
    ...
    def action_show_details(self):
        ...
        return {
            ...
            'context': dict(
                self.env.context,
                # show_lots_m2o=self.has_tracking != 'none' and (picking_type_id.use_existing_lots or move_obj.state == 'done' or move_obj.origin_returned_move_id.id),  # able to create lots, whatever the value of ` use_create_lots`.
                show_lots_m2o=True,
                # show_lots_text=self.has_tracking != 'none' and picking_type_id.use_create_lots and not picking_type_id.use_existing_lots and move_obj.state != 'done' and not move_obj.origin_returned_move_id.id,
...
class CopyStockMove(models.Model):
    ...
    no_mirror_data = fields.Boolean(string="No Mirror Data")
    no_available_lot = fields.Boolean(string="No Available Qty Lot")
