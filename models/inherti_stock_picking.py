...
class InheritStockMove(models.Model):
    ...
    no_mirror_data = fields.Boolean(string="No Mirror Data", default=True)
    is_wh_out = fields.Boolean(string="WH/OUT?", compute='_compute_is_wh_out')
    ...
    def _compute_is_wh_out(self):
        for move in self:
        	if move.picking_id.picking_type_id.id == 4:
        		move.is_wh_out = True
        	else:
        		move.is_wh_out = False

    @api.onchange('product_uom_qty')
    def onchange_product_uom_qty(self):
    	self.update({'no_mirror_data': False})
    	print(self.no_mirror_data)


    @api.model
    def create(self, vals):
    	res = super(InheritStockMove, self).create(vals)
    	print(res.no_mirror_data)
    	if not res.no_mirror_data:
    		# Para setear lotes automaticos, en seleccion manual de productos, # En caso que exista el lote y tenga disponibilidad entonces se procesa todo normal, # en caso de NO cumplir con las condiciones entonces se emite un mensaje de alerta.
    		if res.product_id:
    			...
    			if lot_obj:
    				...
    				for lots in lot_obj:
    					...
    					for avl in quant_obj:
    						available = avl.quantity - avl.reserved_quantity
    						if available >= res.product_uom_qty:
    							count_lots += 1
    							res.move_line_ids.create({
    								'lot_id': lots.id,
    								'lot_name': lots.name,
    								'product_id': res.product_id.id,
    								'product_uom_qty': res.product_qty,
    								'qty_done': res.product_qty,
    							...
    							avl.reserved_quantity += res.product_qty
    						break
    					if count_lots != 0:
    						break
    				if count_lots == 0:
...
#             # Onchange para setear lotes automaticos, en seleccion manual de productos, # En caso que exista el lote y tenga disponibilidad entonces se procesa todo normal, # en caso de NO cumplir con las condiciones entonces se emite un mensaje de alerta.
#             if res.product_id:
#                 lot_obj = self.env['stock.production.lot'].search([('product_id','=',res.product_id.id)])
#                 if lot_obj:
#                     count_lots = 0
